...
BlockController.prototype.checkBlockHash = function(req, res, next) {
  ...
  if (!hash || hash.length < 64 || !isHexadecimal(hash)) {
...
BlockController.prototype.blockHeader = function(req, res, next) {
  ...
  //self.node.services.bitcoind.getBlock(hash, function(err, info) {
		if (err) {
...
BlockController.prototype.blockHeaders = function(req, res, next) {
    ...
    var result = //not supported in colx: self.node.services.bitcoind.getBlockHeaders(hash, cb, nbOfBlockToFetch);
      [self.node.services.bitcoind.getBlockHeader(hash)];
	}else{
		var height = blockIdentifier;
    var result = //self.node.services.bitcoind.getBlockHeaders(height, cb, nbOfBlockToFetch);
      [self.node.services.bitcoind.getBlockHeader(self.node.services.bitcoind.getBlockHash(height))];
	}
...
BlockController.prototype.block = function(req, res, next) {
  ...
  if (blockCached && blockCached.height) {
    blockCached.confirmations = Math.abs(self.node.services.bitcoind.height - blockCached.height + 1);
    console.log("returning cached block: "+blockCached.hash+", height="+blockCached.height+", confirmations="+blockCached.confirmations);
    ...
    self.node.getBlock(hash, function(err, block) {
      ...
      self.node.services.bitcoind.getBlockHeader(hash, function(err, info) {
      //converting raw block is incomplete: self.node.services.bitcoind.getBlock(hash, function(err, info) {
        if (err) {
          return self.common.handleErrors(err, res);
        }
        ...
        self.getBlockReward(blockResult.previousblockhash, function(err,reward) {
          ...
          blockResult.reward = isNaN(reward) ? 0 : reward;
...
BlockController.prototype.transformBlock = function(block, info) {
  //not needed, we already got the block data via getBlockHeader: var blockObj = block.toObject();
  //var transactionIds = blockObj.transactions.map(function(tx) { return tx.hash; });
  //console.log("transformBlock: "+block.hash+", info.height="+info.height+", blockObj.header.height="+blockObj.header.height+", confirmations="+info.confirmations);
  return {
    hash: block.hash,
    size: block.size,//block.toBuffer().length,
    height: info.height ? info.height : block.height,//Obj.header.height,
    version: block.version,//blockObj.header.version,
    merkleroot: block.merkleRoot,//blockObj.header.merkleRoot,
    tx: block.txids,//transactionIds,
    time: block.time,//blockObj.header.time,
    nonce: block.nonce,//blockObj.header.nonce,
    bits: block.bits,//blockObj.header.bits.toString(16),
    difficulty: block.difficulty ? block.difficulty : parseFloat(info.difficulty),
    chainwork: info.chainWork,
    confirmations: Math.abs(info.confirmations ? info.confirmations : block.confirmations),//blockObj.header.confirmations,
    previousblockhash: block.prevHash,//this._normalizePrevHash(blockObj.header.prevHash),
...
BlockController.prototype.blockIndex = function(req, res) {
  ...
  //not working with height and incomplete in colx:getBlockHeader
  this.node.services.bitcoind.getBlock(parseInt(height), function(err, info) {
    if (err) {
      return self.common.handleErrors(err, res);
    }
    res.jsonp({
...
BlockController.prototype._getBlockSummary = function(hash, moreTimestamp, next) {
  ...
  if (summaryCache) {
    ...
    self.node.services.bitcoind.getRawBlock(hash, function(err, blockBuffer) {
      ...
      info.transactions = [];//still broken in v4: [bitcore.Transaction().fromBufferReader(br)];

      self.node.services.bitcoind.getBlockHeader(hash, function(err, blockHeader) {
      //self.node.services.bitcoind.getBlock(hash, function(err, blockHeader) {
        if (err) {
          return next(err);
        }
        ...
        var confirmations = Math.abs(self.node.services.bitcoind.height - height + 1);
...
BlockController.prototype.getPreviousBlock = function(prevHash, cb) {
  ...
  if(prevHash===null) cb(null,null);
      ...
      self.node.getBlock(prevHash, function (err, block) {
          ...
          //self.node.services.bitcoind.getBlock(prevHash, function (err, info) {
              if (err) {
...
BlockController.prototype.getBlockReward = function(prevHash, cb) {
  ...
  self.getPreviousBlock(prevHash, function(err, info) {
    ...
    if(info===null){
      ...
      */
      cb(null, 1000);//https://www.reddit.com/r/ColossuscoinX/comments/7rpusc/colx_staking_amount/
    /*}
  });*/
};
