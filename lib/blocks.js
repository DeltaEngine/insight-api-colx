...
BlockController.prototype.transformBlock = function(block, info) {
  ...
  console.log("transformBlock: "+block.hash+", height="+info.height+", confirmations="+info.confirmations);
  return {
    hash: block.hash,
...
BlockController.prototype.blockIndex = function(req, res) {
  ...
  //not working with height and incomplete in colx:
  this.node.services.bitcoind.getBlockHeader(parseInt(height), function(err, info) {
  /*this.node.services.bitcoind.getBlockHash(parseInt(height), function(err1, hash) {
    if (err1) {
      return self.common.handleErrors(err1, res);
    }
    this.node.services.bitcoind.getBlock(hash, function(err, info) {
      */
      if (err) {
        return self.common.handleErrors(err, res);
      }
