...
TxController.prototype.transformTransaction = function(transaction, options, callback) {
  ...
  var confirmations = 1;//fix to help with everything being unconfirmed: 0;
  if(transaction.height >= 0) {
    confirmations = Math.abs(this.node.services.bitcoind.height - transaction.height + 1);
  }
  ...
  if (!transformed.blockheight || !(transaction.height >= 0)) {
    //fix still broken tx by grabbing height and confirmation from the block hash
    this.node.services.bitcoind.getBlockHeader(transformed.blockhash, function(err, header) {
      transformed.blockheight = header.height;
      transformed.confirmations = Math.abs(header.confirmations);
...
TxController.prototype.transformInput = function(options, input, index) {
  ...
  transformed.confirmations = Math.abs(input.confirmations || 1);//null; // TODO
