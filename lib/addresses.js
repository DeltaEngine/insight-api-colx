...
AddressController.prototype.transformUtxo = function(utxoArg) {
  var utxo = {
    ...
    amount: (utxoArg.satoshis || 0) / 1e8,
    satoshis: utxoArg.satoshis,
    confirmations = 1//tst to fix everything being unconfirmed: 0;
  };
  try{
    if (utxoArg.height && utxoArg.height > 0) {
      utxo.height = utxoArg.height;
      utxo.confirmations = Math.abs(this.node.services.bitcoind.height - utxoArg.height + 1);
    }
    ...
    console.log("Failed to transform utxo: "+e);
  }
  return utxo;
};
